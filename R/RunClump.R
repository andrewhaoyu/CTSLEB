...
RunClump <- function(plink19_exec=plink19_exec,
  ...
  for(r_ind in 1:length(r2_vec)){
    ...
    for(w_ind in 1:length(wc_vec)){
      ...
      r2thr <- r2_vec[r_ind]
      kbpthr <- wc_vec[w_ind]
      ref_outfile <- paste0(temp.dir,"ref_", outprefix, "_CT_rind_",r_ind,"_wcind_",w_ind)
      target_outfile <- paste0(temp.dir,"target_", outprefix, "_CT_rind_",r_ind,"_wcind_",w_ind)
